...
public class ExternalApiClient {
        ...
        public Mono<CurrencyRates> getCurrencyRates(String baseCurrency) {
                ...
                // .map(json -> {
                // // json string burada mevcut
                // System.out.println("Fetched JSON in map: " + json);
                // return json;
                // })
                // .subscribe(json -> {
                // // Asenkron olarak geldiğinde işleme
                // System.out.println("Fetched JSON in subscribe: " + json);
                // });
        }
        ...
        public Mono<CurrencyRates> getHistoricalRates(String baseCurrency, LocalDate date) {
                ...
                var response = webClient.mutate()
                                ...
                                .retrieve()
                                .bodyToMono(CurrencyRates.class);
                // .doOnNext(r -> log.info("Fetched response: {}", r))
                // .doOnError(error -> log.error("Error fetching historical rates for {} on {}:
                // {}",
                // baseCurrency, dateStr, error.getMessage()));

                // CurrencyRates result = response.block();
                // System.out.println("DATE: " + result.getDate());
                // System.out.println("RATES: " + result.getRates());
